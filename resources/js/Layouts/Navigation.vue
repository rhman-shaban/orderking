<template>
    ...
    <div :class="$page.props.showingMobileMenu ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
        ...
        <nav class="mt-10" x-data="{ isMultiLevelMenuOpen: false }">
            ...
            <nav-link v-if="$page.props.auth.guard === 'merchant' || $page.props.auth.guard === 'admin'" :href="route('users.index')" :active="route().current('users.index')">
                <template #icon>
            ...
            <nav-link v-if="$page.props.auth.guard === 'admin'" :href="route('merchants.index')" :active="route().current('merchants.index')">
                <template #icon>
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                         xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
